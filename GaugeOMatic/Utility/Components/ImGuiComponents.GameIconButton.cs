...
public static partial class ImGuiComponents
{
    ...
    /// <returns></returns>
    public static bool GameIconButton(uint gameIcon, string buttonId, Vector2 iconSize, Vector4? defaultColor = null, Vector4? activeColor = null, Vector4? hoveredColor = null) => GameIconButton(gameIcon, buttonId, iconSize, iconSize + ImGui.GetStyle().FramePadding, defaultColor, activeColor, hoveredColor);
    ...
    internal static bool GameIconButtonSelect<T>(string label, ref T val, IEnumerable<uint> optionIcons, IEnumerable<T> optionValues, Vector2 iconSize, Vector2? buttonSize = null, uint columns = 0, Vector4? defaultColor = null, Vector4? activeColor = null, Vector4? hoveredColor = null)
    {
        var options = optionIcons.Zip(optionValues, static (icon, value) => new KeyValuePair<uint, T>(icon, value));
        return GameIconButtonSelect(label, ref val, options, iconSize, buttonSize, columns, defaultColor, activeColor, hoveredColor);
    }
    ...
    /// <param name="hoveredColor">The color of the buttons when hovered.</param>
    /// <returns>True if any button is clicked.</returns>
    internal static unsafe bool GameIconButtonSelect<T>(string label, ref T val, IEnumerable<KeyValuePair<uint, T>> options, Vector2 iconSize, Vector2? buttonSize = null, uint columns = 0, Vector4? defaultColor = null, Vector4? activeColor = null, Vector4? hoveredColor = null)
    {
        ...
        for (var i = 0; i < optArr.Length; i++)
        {
            ...
            if (GameIconButton(icon, $"{label}{option}{i}", iconSize, buttonSize.Value, selected ? activeColor : defaultColor, activeColor, hoveredColor))
            {
                val = option;
                result = true;
            }
